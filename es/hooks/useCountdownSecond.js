...
var useCountdownSecond = function useCountdownSecond(_temp) {
  ...
  // 基本信息
  var startDate = useRef(0);
  var endDate = useRef(0);
  var remainSecond = useRef(0); // 倒计时

  var _useSaferState = useSaferState(0),
      reciprocal = _useSaferState[0],
      setReciprocal = _useSaferState[1]; // 是否倒计时已经结束


  var isReciprocalClose = useRef(true); // 是否暂停

  var isPause = useRef(false); // 开始倒计时
